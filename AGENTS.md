...
## Contrato API
```
POST   /api/auth/register | /api/auth/login          # JWT httpOnly cookie (access 15m + refresh 7d)
POST   /api/auth/refresh | DELETE /api/auth/logout    # refresh rota, logout limpia cookies
GET    /api/contenidos/buscar?tipo=&q=                # UC1 externo (rate-limited 20/min, auth cookie|Bearer)
...
- UC2: check `@@unique` antes de `create`; crea `UsuarioContenido(pendiente)` y opcional `ListaContenido`, race protegido via P2002 catch → reuse.
